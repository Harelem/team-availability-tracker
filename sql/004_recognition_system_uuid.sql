-- Recognition System Migration (UUID Compatible)
-- Creates tables and functions for user achievements and recognition metrics
-- Fixed for databases where team_members.id is UUID type
...
CREATE POLICY "Users can view their own achievements and team achievements" ON user_achievements
  FOR SELECT USING (
    ...
    user_id IN (
      ...
      WHERE tm.team_id IN (
        SELECT team_id FROM team_members WHERE id = auth.uid()
      )
    )
  );
...
-- ============================================================================
-- RECOGNITION CALCULATION FUNCTIONS (Updated for UUID)
-- ============================================================================

-- Function to calculate weekly completion rate (Updated for UUID)
CREATE OR REPLACE FUNCTION calculate_weekly_completion_rate(user_id_param UUID, week_start_param DATE)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Function to calculate user recognition metrics (Updated for UUID)
...
-- Function to check and award achievements (Updated for UUID)
...
-- Function to get team recognition leaderboard (Updated for UUID)
...
COMMENT ON TABLE user_achievements IS 'Stores user achievements and recognition badges earned through consistent planning behavior (UUID Compatible)';
...
COMMENT ON TABLE recognition_metrics IS 'Stores calculated recognition metrics for users across different time periods (UUID Compatible)';
...
COMMENT ON FUNCTION calculate_user_recognition_metrics() IS 'Calculates and updates recognition metrics for all users based on their schedule entries (UUID Compatible)';
COMMENT ON FUNCTION check_user_achievements(UUID) IS 'Checks and awards new achievements for a specific user based on their current metrics (UUID Compatible)';
COMMENT ON FUNCTION get_team_recognition_leaderboard(TEXT) IS 'Returns team-wide recognition leaderboard data for specified timeframe (week, month, quarter) (UUID Compatible)';