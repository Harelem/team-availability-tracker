-- Recognition System Migration (INTEGER Compatible with Fixed RLS)
-- Creates tables and functions for user achievements and recognition metrics
-- Fixed for databases where team_members.id is INTEGER type
...
-- RLS Policies (Fixed - No auth.uid() casting)
-- Allow all authenticated users to read achievements (simplified for now)
CREATE POLICY "Allow authenticated users to view achievements" ON user_achievements
  FOR SELECT USING (auth.role() = 'authenticated');

-- Allow system operations for creating achievements
CREATE POLICY "Allow system to create achievements" ON user_achievements
  FOR INSERT WITH CHECK (true);

-- Allow system operations for updating achievements  
CREATE POLICY "Allow system to update achievements" ON user_achievements
...
-- RLS Policies (Fixed - No auth.uid() casting)
-- Allow all authenticated users to read metrics (simplified for now)
CREATE POLICY "Allow authenticated users to view metrics" ON recognition_metrics
  FOR SELECT USING (auth.role() = 'authenticated');

-- Allow system operations for managing metrics
CREATE POLICY "Allow system to manage recognition metrics" ON recognition_metrics
...
BEGIN
  ...
  WHERE se.member_id = user_id_param
...
COMMENT ON TABLE user_achievements IS 'Stores user achievements and recognition badges earned through consistent planning behavior (INTEGER Compatible)';
...
COMMENT ON TABLE recognition_metrics IS 'Stores calculated recognition metrics for users across different time periods (INTEGER Compatible)';
...
COMMENT ON FUNCTION calculate_user_recognition_metrics() IS 'Calculates and updates recognition metrics for all users based on their schedule entries (INTEGER Compatible)';
COMMENT ON FUNCTION check_user_achievements(INTEGER) IS 'Checks and awards new achievements for a specific user based on their current metrics (INTEGER Compatible)';
COMMENT ON FUNCTION get_team_recognition_leaderboard(TEXT) IS 'Returns team-wide recognition leaderboard data for specified timeframe (week, month, quarter) (INTEGER Compatible)';